...
function MissionMonitor() {
  ...
  // Initialize from mission data when loaded
  useEffect(() => {
    if (mission) {
      // Sync speed
      if (mission.parameters?.speed) {
        simulation.setSpeed(mission.parameters.speed);
      }
      // Sync flight pattern
      if (mission.flight_pattern) {
        const pattern = Object.values(simulation.MissionType).find(
          t => t.toLowerCase() === mission.flight_pattern.toLowerCase()
        );
        if (pattern) {
          simulation.setMissionType(pattern);
        }
      }
    }
  }, [mission]);
  ...
  const createDroneIcon = useCallback((heading) => {
    const isFlying = simulation.droneState === simulation.DroneState.FLYING ||
      simulation.droneState === simulation.DroneState.TAKEOFF ||
      simulation.droneState === simulation.DroneState.RTH;

    return L.divIcon({
      className: `drone-marker-icon ${isFlying ? 'flying' : ''}`,
      html: `
        <div class="drone-icon-wrapper" style="transform: rotate(${heading}deg)">
          ...
          >
            ...
            {simulation.position && isFinite(simulation.position[0]) && isFinite(simulation.position[1]) && (
              <Marker
                ...
                icon={createDroneIcon(simulation.heading)}
              />
            )}
          </MapContainer>
